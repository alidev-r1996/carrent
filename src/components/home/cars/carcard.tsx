  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        viewport={{ once: true }}
        className="childBlur transition-all duration-300 rounded cursor-pointer shadow p-2 flex flex-col gap-2 text-sm bg-header justify-between w-full min-w-56 sm:min-w-60"
      >
        <div className="flex items-start justify-between p-2">
          <div className="flex flex-col gap-.5">
            <h2 className="font-bold dark:text-slate-200">{name}</h2>
            <p className="text-slate-400 text-[10px] dark:text-slate-300">{type}</p>
          </div>
          <p className={`w-5 h-5 ${isLike ? 'text-rose-500':"text-rose-50 dark:text-transparent"}`}>
        ...
        <div className="flex items-start justify-between p-2 ">
          <div className="flex flex-col">
            <p className="text-[10px] flex-1 text-slate-500">
              <strong className=" text-sm text-black dark:text-slate-200">${price}/</strong> days
            </p>
            {discount && <p className="text-slate-400 line-through text-xs w-full">${discount}</p>}
          </div>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
